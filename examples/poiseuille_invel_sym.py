...
- flow driven by inlet velocity
- fixed density at the outlet
- periodic bc in the flow direction (as well as in the "dummy" direction)
- wall bc (immobile & adiabatic) for representing the lower plate
- symmetry bc for mirroring the flow field about the channel centreline
...
from jyulb.flow_field import poise_plate_steady_state_umax
# ---------------------------------------------------------------------------
...
H = 6  # Half of the distance between the plates
...
centrel_dist = np.zeros((2*H), dtype=np.float64)
ana_vel = np.zeros((2*H), dtype=np.float64)

max_vel = 1e-3
poise_plate_steady_state_umax(2*H, max_vel, centrel_dist, ana_vel)
...
# Set inlet velocity (used both as initial and boundary value)
vel_dir_cff = 1.0
vind = np.zeros((4), dtype=np.int32)
vind[3] = FLOW_DIR

if INLET in (FACE_X1,FACE_Y1,FACE_Z1):
    vel_dir_cff = -1.0
    vind[FLOW_DIR] = L-1

for index in np.ndindex((W,H)):
    vind[DUMMY_DIR] = index[0]
    vind[PLATE_DIR] = index[1]
    vel[tuple(vind)] = vel_dir_cff*ana_vel[index[1]]
...
with open('flow_prof.txt', 'w') as f:
    for h in range(H):
        ...
        sim_ana_diff = sim_loc_vel[FLOW_DIR] - vel_dir_cff*ana_loc_vel
