...
# ---------------------------------------------------------------------------
# Geometric setup, location of the plate and the centreline
CENTRE_LINE = FACE_X1
LOWER_PLATE = FACE_1X

PLATE_DIR = X # The plates are separated in this direction
DUMMY_DIR = Y # The "dummy" direction
...
W = 4  # Width of the domain in the "dummy" direction
...
# ---------------------------------------------------------------------------
# Simulation geometry
phase = np.full(lat_size,FLUID,dtype=np.uint8)
# Initial density field (reduced densities)
iden = np.ones(lat_size,dtype=np.float64)
# Initial velocity field
ivel = np.zeros((lat_size[X],lat_size[Y],lat_size[Z],3),dtype=np.float64)

solver = Solver.with_iflow(phase, iden, ivel, rlx_scheme, kvisc, g, bc)
...
dvisc = rden*kvisc
eff_pgrad = -rden*grav
poise_plate_steady_state_pgrad(2*H, dvisc, eff_pgrad, centrel_dist, ana_vel)
